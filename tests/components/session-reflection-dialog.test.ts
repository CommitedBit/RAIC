...
describe('SessionReflectionDialog', () => {
  ...
  it('shows governed co-thinking agency prompts without adding persistence fields', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          ...
          context: {
            ...
            reflectionSummary: 'Learners practiced intent before prompting.',
            masteryHints: ['verification', 'transfer'],
            confidenceScore: 3,
          },
          reflections: [
            {
              summary: 'Learners practiced intent before prompting.',
    ...
    expect(container.textContent).toContain('Governed Co-Thinking Reflection');
    expect(container.textContent).toContain('teacher-only agency practice notes');
    expect(container.textContent).toContain('What no-AI transfer task');
    expect(
      container.querySelector('[data-testid="governed-co-thinking-reflection-template"]'),
    ).toBeTruthy();

    const summary = container.querySelector('textarea');
    const challengingAreas = container.querySelector('input[placeholder*="agency habits"]');
    expect(summary?.getAttribute('placeholder')).toContain('class-level agency evidence');
    expect(challengingAreas).toBeTruthy();
