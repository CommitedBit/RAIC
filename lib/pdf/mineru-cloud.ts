 ...
 * MinerU Cloud API (v4).
 *
 * Flow: create batch upload URL, PUT the PDF, poll extraction status, then
...
function isRetryable(err: unknown): boolean {
  ...
  return ['fetch failed', 'econnreset', 'etimedout', 'timeout', 'aborted'].some((text) =>
    msg.includes(text),
  );
}
...
async function parseMinerUZip(zipUrl: string): Promise<ParsedPdfContent> {
  ...
  const fullMdPath = filePaths.find((path) => /(^|\/)full\.md$/i.test(path));
  const contentListPath = filePaths.find(
    (path) => path.endsWith('_content_list.json') || /(^|\/)content_list\.json$/i.test(path),
  );
