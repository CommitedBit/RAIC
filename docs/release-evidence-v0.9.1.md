...
Evidence status: complete. The clean Node 24 gate, protected preview review, production deployment, live homepage checks, production smokes, immutable tag, and GitHub release publication were verified at release time.
...
- [x] `https://open-raic.com` rendered the simplified homepage at all three target viewports with no horizontal overflow, no visible legacy demo or empty-schedule copy, and only the screen-reader page heading
...
- [x] `/api/health` returned HTTP 200 with version `0.9.1`; source documents, image generation, auth, Discord, encryption, and Postgres were ready while web search, video, TTS, and MiroFish remained unavailable
...
- On 2026-08-01, the live endpoint still served version `0.9.1`, but durable Postgres readiness had degraded because the linked Neon resource exceeded its compute quota.
- The release-time proof above remains historical evidence, not a claim of current readiness. Database migration and restored live auth/storage proof are tracked in the `v0.9.2` evidence.
